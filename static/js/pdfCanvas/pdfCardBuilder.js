...
import { openAnnotationInput, startWhiteout, formatDate, getFormFieldMetrics, getSortedFormFields, findAnnotationForField } from './pdfAnnotations.js';
import { startCrop, handleOverlayClick, removeOverlay } from './pdfCropOverlay.js';
import { renderCardCanvas } from './pdfRender.js';
import { snapshotPageLayers, snapshotPageExcluded, snapshotPagesOrder, snapshotPageFormFields, snapshotAllFormFields } from './pdfHistory.js';

// ── Field-form confirmation dialog ─────────────────────────────────────────
...
export function renderCardLayers(wrap, page) {
    ...
    if (page.formFields && page.formFields.length > 0) {
        // Sort fields row by row: left-to-right, top-to-bottom for Tab navigation
...
export function buildCard(page) {
    ...
    wrap.addEventListener('mousedown', (e) => {
        if (e.target.closest('.crop-rect') || e.target.closest('.overlay-rect') || e.target.closest('.ann-edit-box') || e.target.closest('.whiteout-rect') || e.target.closest('.pdf-form-field-creating')) return;
        if (state.mode === 'crop') {
            e.preventDefault();
